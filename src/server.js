 ...
 * @returns {Promise<object>} Fastify 实例。
 */
export async function buildServer(dependencies = {}) {
  const repo = dependencies.repo
  if (!repo) throw new Error('repo is required')
  const receiver = dependencies.receiver || new Service115()
  const organizer = dependencies.organizer || new Organizer115()
  const telegram = dependencies.telegram || new TelegramService({ repo })
  const adminPassword = dependencies.adminPassword ?? ADMIN_PASSWORD
  ...
  const readAsset = dependencies.readAsset || readFile
  ...
  app.get('/', async (_req, reply) => {
    ...
    return readAsset(join(publicDir, 'index.html'), 'utf8')
  })
  app.get('/app.js', async (_req, reply) => {
    reply.type('application/javascript; charset=utf-8')
    return readAsset(join(publicDir, 'app.js'), 'utf8')
  })
  app.post('/api/admin/login', async (req, reply) => {
    const body = z.object({ password: z.string() }).parse(req.body || {})
    if (body.password !== adminPassword) return reply.code(401).send({ ok: false, error: '密码错误' })
  ...
  app.get('/api/settings', { preHandler: requireAuth }, async () => ({
    ok: true, dataDir, settings: maskSettings(repo.getSettings()), accounts: await organizer.listAccounts(),
  }))
  app.put('/api/settings', { preHandler: requireAuth }, async (req) => {
    const body = z.object({
      cookie115: z.string().optional(), telegramBotToken: z.string().optional(), telegramChatId: z.string().optional(),
      telegramNotifyChatId: z.string().optional(), targetRootCid: z.string().optional(),
      clearCookie115: z.boolean().optional(), clearTelegramBotToken: z.boolean().optional(),
    }).parse(req.body || {})
    const current = repo.getSettings()
    const next = { ...current }
    for (const [key, value] of Object.entries(body)) {
      if (key === 'clearCookie115' || key === 'clearTelegramBotToken' || value === undefined) continue
  ...
  app.post('/api/settings/test-115-cookie', { preHandler: requireAuth }, async () => ({ ok: true, info: await receiver.getUserInfo(repo.getSettings().cookie115 || '') }))
  app.post('/api/settings/test-telegram', { preHandler: requireAuth }, async () => {
    ...
    const res = await http.get(`https://api.telegram.org/bot${token}/getMe`, { timeout: 8000 })
  ...
  app.post('/api/settings/115-oauth/start', { preHandler: requireAuth }, async () => {
    ...
    organizer.startQrLogin({ onQrCode: async (qrcode) => { oauth115.qrcode = qrcode }, timeoutMs: 120000 })
      .then((account) => Object.assign(oauth115, { running: false, status: 'success', account }))
      .catch((error) => Object.assign(oauth115, { running: false, status: 'failed', error: error.message }))
    for (let i = 0; i < 50; i++) {
  ...
  app.get('/api/settings/115-oauth/status', { preHandler: requireAuth }, async () => ({ ok: true, ...oauth115, accounts: await organizer.listAccounts() }))
  app.post('/api/settings/test-115-oauth', { preHandler: requireAuth }, async () => {
    const token = await organizer.getToken()
    return { ok: true, user: token.user_id || token.accountId || '115', rootItems: (await organizer.list('0')).length }
  })
  app.get('/api/tasks', { preHandler: requireAuth }, async (req) => {
    const requested = Number(req.query?.limit || 100)
    const limit = Number.isFinite(requested) ? Math.max(0, Math.min(requested, 500)) : 100
  ...
  app.post('/api/tasks/manual', { preHandler: requireAuth }, async (req) => {
    ...
    const tasks = parse115ShareLinks(body.text).map((link) => repo.upsertTask({ ...link, source: 'manual' }))
    return { ok: true, count: tasks.length, tasks }
  })
  app.post('/api/tasks/:id/retry', { preHandler: requireAuth }, async (req) => {
    const id = Number(req.params.id)
    if (!repo.getTask(id)) return { ok: false, error: '任务不存在' }
...
/**
 * 创建生产依赖并启动服务。
 * @returns {Promise<{app: object, shutdown: Function}>} 服务与关闭函数。
 */
export async function startServer() {
  ...
  const telegram = new TelegramService({ repo })
  ...
  const app = await buildServer({ repo, receiver, organizer, telegram, logger: true })
